...
// DangerousHackReadPacketData() is ZeroCopyReadPacketData(), but you also
// pass in the slice that gets filled in with info form the PCAP C buffer.
// All the warnings about ZeroCopyReadPacketData() apply here, PLUS this
// is a function that we wrote - it does not appear in the official
// google/gopacket library.
// See: https://github.com/golang/go/wiki/cgo#turning-c-arrays-into-go-slices
// for further guidance and explanations.
func (p *Handle) DangerousHackReadPacketData(data *[]byte) (gopacket.CaptureInfo, error) {
	var ci gopacket.CaptureInfo
	var err error
	p.mu.Lock()
	err = p.getNextBufPtrLocked(&ci)
	if err == nil {
		// Turn the argument data []byte into pointer to a SliceHeader.
		// No allocations here - essentially just a "cast" to another type.
		slice := (*reflect.SliceHeader)(unsafe.Pointer(data))
		// Set the buffer that the slice data []byte uses to an address in the
		// buffer that the PCAP library uses. This is *not* memory allocated via
		// Go runtime, either explicitly via make() or new() or by the compiler
		// when it does escape analysis. It's probably from a malloc() deep in the
		// PCAP C code. It might be a problem if data []byte gets garbage collected:
		// the Go GC algorithm would get a pointer to some memory that it never allocated.
		// So make sure that data []byte never gets garbage collected.
		// Also you should only use that underlying memory for the slice until the next
		// call to p.getNextBufPtrLocked(), whereupon it turns into a pumpkin or something,
		// even if data []byte points to it.
		slice.Data = uintptr(unsafe.Pointer(p.bufptr))
		slice.Len = ci.CaptureLength
		slice.Cap = ci.CaptureLength
	}
